...
{
	uint64_t result = 0;
	uint64_t const width = input.size();
	for(auto x=0;x<width;++x)
	{
		uint64_t const height = input[x].length();
...
uint64_t secondPart(Text const& input)
{
	...
	{	
		...
		for(auto y=0;y<height;++y)
		{
			if(input[x][y]=='A')
			{
				bool const boundsCheckLeft = !outOfBounds(x-1,y-1,width,height) && !outOfBounds(x-1,y+1,width,height);
				bool const boundsCheckRight = !outOfBounds(x+1,y-1,width,height) && !outOfBounds(x+1,y+1,width,height);
				...
				if (boundsCheckLeft && boundsCheckRight)
				{
					...
					bool const sameRows = (chars[0] == chars[2]) &&  (chars[1] == chars[3]);
					bool const sameColumns = (chars[0] == chars[1]) &&  (chars[2] == chars[3]);
